...
def _validate_specification(specification: Any) -> None:
    ...
    for field in (
        "requirements", "constraints", "inputs", "outputs", "acceptance_criteria",
    ...
    for collection_name in (
        "requirements", "constraints", "inputs", "outputs", "acceptance_criteria", "assumptions",
    ):
        for item in specification[collection_name]:
            ...
            if item.get("origin") not in ALLOWED_ORIGINS:
                raise ValueError(
                    f"unsupported origin in {collection_name}: {item.get('origin')!r}"
                )


def _step_for_requirement(
    specification: dict[str, Any], requirement: dict[str, Any], sequence: int
) -> dict[str, Any]:
...
def transform_specification_to_build_plan(specification: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in specification["provenance"]:
        ...
        if element_id is None:
            continue
        for step in steps:
            if step["origin"] != "UNRESOLVED" and any(
                p["source_specification_element_id"] == element_id
    ...
    plan_provenance.sort(
        key=lambda item: (item["source_specification_element_id"], item["build_plan_element_id"])
    )

    status = "BLOCKED" if blockers else (
        "READY_FOR_APPROVAL" if approval_required else "VALIDATED"
    )
